...
u8 nRF24L01_Check (void)
{
	...
	bsp_set_spi1_speed (SPI_SPEED_8); // spi speed 9Mhz (24L01 maximum SPI clock is 10Mhz)
...
u8 nRF24L01_Write_Reg (u8 reg, u8 value)
{
	...
	status = bsp_readwritebyte_spi1 (reg); // send the register number
	bsp_readwritebyte_spi1 (value); // write registers
...
u8 nRF24L01_Read_Reg (u8 reg)
{
	...
	bsp_readwritebyte_spi1 (reg); // send the register number
	reg_val = bsp_readwritebyte_spi1 (0XFF); // read register
...
u8 nRF24L01_Read_Buf (u8 reg, u8 * pBuf, u8 len)
{
	...
	status = bsp_readwritebyte_spi1 (reg); // send the register values (location), and read the status value
	for (u8_ctr=0; u8_ctr<len; u8_ctr++) 
		pBuf[u8_ctr] = bsp_readwritebyte_spi1(0XFF); // read data
...
u8 nRF24L01_Write_Buf (u8 reg, u8 * pBuf, u8 len)
{
	...
	for (u8_ctr=0;u8_ctr<len;u8_ctr++) 
		bsp_readwritebyte_spi1(*pBuf++); // write data
